    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                ...
                await _userManager.AddClaimAsync(user, new System.Security.Claims.Claim("http://shemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress", user.Email));
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        await _signInManager.SignInAsync(user, isPersistent: false);
                        _logger.LogInformation(6, "User created an account usin {Name} provider.", info.LoginProvider);
                        return RedirectToAction("Dashboard", "Dashboard");
                    }
                }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null || !(await _userManager.IsEmailConfirmedAsync(user)))
                {
                    ...
                    return View("ForgotPasswordEmailConfirmation");
                }
